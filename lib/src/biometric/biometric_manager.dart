...
class BiometricManager {
  ...
  Future<bool> addFingerprint({String? fingerprintHash}) async {
    // 先进行身份验证
    bool isAuthenticated = await authenticate(reason: '请验证您的身份以添加新指纹');
  ...
  Future<bool> deleteFingerprint(int index) async {
    // 先进行身份验证
    bool isAuthenticated = await authenticate(reason: '请验证您的身份以删除指纹');
  ...
  Future<bool> clearAllFingerprints() async {
    // 先进行身份验证
    bool isAuthenticated = await authenticate(reason: '请验证您的身份以清除所有指纹');
